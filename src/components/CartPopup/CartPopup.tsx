...
import { Table, Text, Group, ActionIcon, ScrollArea, Image  } from '@mantine/core';
...
const CartPopup: React.FC<CartPopupProps> = ({}) => {
  ...
  const rows = items.map(item => (
    <Table.Tr  key={item.id}>
      <Table.Td>
        <Group gap="sm">
          <img src={item.image} alt={item.name} style={{ width: 40, height: 40 }} />
  ...
  return (
    <div className={classes.popup}>
      <ScrollArea style={{ maxHeight: 5000 }}>
        <Table>
          <Table.Tbody>
            {rows}
          </Table.Tbody>
